...
(function (root) {
  ...
  var COL = {
    line:    'rgba(126,182,224,.30)',
    lineDim: 'rgba(126,182,224,.15)',
    us:      '#3aa8c4',
    usLabel: '#04121a',
    them:    '#FF5A6E',
  ...
  function fit(W, H, opts) {
    opts = opts || {};
    var crop = CROPS[opts.crop] || CROPS.full;
  ...
  function pitch(ctx, f, o) {
    ...
    // mowing stripes — subtle tonal bands along the pitch length (goal to goal)
    if (o.stripes !== false) {
      var NSTR = 14;
      for (var si = 0; si < NSTR; si++) {
        var sa = f.px(si / NSTR, 0), sb = f.px((si + 1) / NSTR, 1);
        ctx.fillStyle = si % 2 ? 'rgba(150,190,225,.028)' : 'rgba(0,0,0,.055)';
        ctx.fillRect(Math.min(sa[0], sb[0]), Math.min(sa[1], sb[1]), Math.abs(sb[0] - sa[0]), Math.abs(sb[1] - sa[1]));
      }
    }
    ...
    var arc = function (cx, cy, rm, a0, a1) { var C = f.px(cx, cy); ctx.beginPath(); ctx.arc(C[0], C[1], f.m(rm), a0, a1); ctx.stroke(); };
    // orientation-aware arc: fb = field-space bisector angle (x right, y down), half = half sweep
    var arcF = function (cx, cy, rm, fb, half) { var C = f.px(cx, cy), P = f.px(cx + Math.cos(fb) * 0.02, cy + Math.sin(fb) * 0.02); var sa = Math.atan2(P[1] - C[1], P[0] - C[0]); ctx.beginPath(); ctx.arc(C[0], C[1], f.m(rm), sa - half, sa + half); ctx.stroke(); };
    ...
    // outer boundary — brighter crisp frame
    ctx.save();
    ctx.strokeStyle = 'rgba(150,196,232,.44)'; ctx.lineWidth = Math.max(1.2, f.m(0.19));
    rect(0, 0, 1, 1);
    ctx.restore();
    ...
    arcF(spot, 0.5, 9.15, 0, 0.927);
    // their end (x1)
    rect(1, pbW0, 1 - pd, pbW1); rect(1, sbW0, 1 - sd, sbW1); dot(1 - spot, 0.5);
    arcF(1 - spot, 0.5, 9.15, Math.PI, 0.927);
  ...
  function token(ctx, f, nx, ny, o) {
    ...
    var acc = o.accent || '#A78BFA';
    var base = o.kind === 'carrier' ? acc : (o.kind === 'keeper' ? '#8fd8e6' : (o.fill || COL.us));
    var ringOn = o.best || o.kind === 'best';
    var edge = ringOn ? acc : base;
    var cy = y - r * 0.14, s = r * 1.94, hx = x - s / 2, hy = cy - s / 2, rad = s * 0.3;
    var a0 = o.alpha == null ? 1 : o.alpha;
    ctx.save(); ctx.globalAlpha = a0; ctx.lineJoin = 'round';
    // free-man halo — rounded-square, pulsing
    if (o.free || o.kind === 'free') {
      var pulse = o.pulse == null ? 0 : o.pulse, gap = 5 + pulse * 3;
      ctx.save(); ctx.strokeStyle = COL.free; ctx.globalAlpha = (0.4 + 0.5 * pulse) * a0;
      ctx.lineWidth = Math.max(2, f.m(0.08));
      roundRect(ctx, hx - gap, hy - gap, s + gap * 2, s + gap * 2, rad + gap * 0.7); ctx.stroke(); ctx.restore();
    }
    // ground shadow
    ctx.fillStyle = 'rgba(0,0,0,.3)'; ctx.beginPath(); ctx.ellipse(x, cy + s * 0.52, s * 0.42, s * 0.15, 0, 0, 7); ctx.fill();
    // HUD chip — translucent gradient fill
    var g = ctx.createLinearGradient(hx, hy, hx, hy + s);
    g.addColorStop(0, hexA(base, o.kind === 'carrier' ? 0.38 : 0.24));
    g.addColorStop(1, hexA(base, o.kind === 'carrier' ? 0.14 : 0.07));
    ctx.fillStyle = g; roundRect(ctx, hx, hy, s, s, rad); ctx.fill();
    // inner hairline
    ctx.strokeStyle = hexA(base, 0.28); ctx.lineWidth = 1; roundRect(ctx, hx + 2, hy + 2, s - 4, s - 4, rad - 2); ctx.stroke();
    // glowing outline
    ctx.strokeStyle = hexA(edge, 0.95); ctx.lineWidth = Math.max(1.5, f.m(ringOn ? 0.085 : 0.055));
    ctx.shadowColor = hexA(edge, 0.85); ctx.shadowBlur = ringOn ? 13 : 8;
    roundRect(ctx, hx, hy, s, s, rad); ctx.stroke(); ctx.shadowBlur = 0;
    // corner brackets — digital target read
    var tk = s * 0.2, ins = rad * 0.42;
    ctx.strokeStyle = hexA(edge, 0.95); ctx.lineWidth = Math.max(1.3, f.m(0.055)); ctx.lineCap = 'round';
    [[hx + ins, hy + ins, 1, 1], [hx + s - ins, hy + ins, -1, 1], [hx + ins, hy + s - ins, 1, -1], [hx + s - ins, hy + s - ins, -1, -1]].forEach(function (c) {
      ctx.beginPath(); ctx.moveTo(c[0] + c[2] * tk, c[1]); ctx.lineTo(c[0], c[1]); ctx.lineTo(c[0], c[1] + c[3] * tk); ctx.stroke();
    });
    ctx.lineCap = 'butt';
    // label — bright, glowing
    if (o.role) {
      ctx.fillStyle = '#EAF6FB'; ctx.shadowColor = hexA(edge, 0.9); ctx.shadowBlur = 6;
      ctx.font = "800 " + (o.font || f.font) + "px 'IBM Plex Mono'";
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(o.role, x, cy); ctx.shadowBlur = 0; ctx.textBaseline = 'alphabetic';
    }
    // facing notch
    if (o.facing != null) {
      var fx = x + Math.cos(o.facing) * s * 0.5, fy = cy + Math.sin(o.facing) * s * 0.5;
      ctx.fillStyle = hexA(edge, 1); ctx.shadowColor = hexA(edge, 0.9); ctx.shadowBlur = 6;
      ctx.beginPath(); ctx.arc(fx, fy, Math.max(2, f.m(0.13)), 0, 7); ctx.fill(); ctx.shadowBlur = 0;
    }
    ctx.restore();
    return [x, cy];
  }
  ...
  function chevron(ctx, f, nx, ny, o) {
    ...
    var col = o.color || COL.them;
    ctx.beginPath(); ctx.moveTo(-r, 0); ctx.lineTo(r * 0.55, -r * 0.85); ctx.lineTo(r * 0.15, 0); ctx.lineTo(r * 0.55, r * 0.85); ctx.closePath();
    ctx.fillStyle = hexA(col, 0.24); ctx.fill();
    ctx.strokeStyle = hexA(col, 0.95); ctx.lineWidth = Math.max(1.4, f.m(0.06)); ctx.lineJoin = 'round';
    ctx.shadowColor = hexA(col, 0.85); ctx.shadowBlur = 7; ctx.stroke(); ctx.shadowBlur = 0;
    ctx.restore();
    if (o.role) {
      ctx.save(); ctx.globalAlpha = o.alpha == null ? 1 : o.alpha;
  ...
  /* ---- PENALTY POV (shooter's-eye, goal at NORTH) --------------------------
   * Tilted-camera perspective looking down the spot toward the goal. No tokens.
   * o.elev 0..1 = camera height (normal eye-level..raised); o.cam = metres the
   * camera sits behind the goal line (smaller = closer / bigger goal). */
  function penaltyPOV(ctx, W, H, o) {
    ...
    sg.addColorStop(0, '#04070e'); sg.addColorStop(1, '#08111f');
    ctx.fillStyle = sg; ctx.fillRect(0, 0, W, yHoriz + 1);
    var gg = ctx.createLinearGradient(0, yHoriz, 0, H);
    gg.addColorStop(0, '#0b1a2b'); gg.addColorStop(0.5, '#0d1e31'); gg.addColorStop(1, '#050d18');
    ctx.fillStyle = gg; ctx.fillRect(0, yHoriz, W, H - yHoriz);
    // perspective mowing bands (constant-depth → horizontal), capped at the camera
    for (var d = 0; d < maxD; d += 2) {
      var y0 = yAt(d), y1 = yAt(Math.min(maxD, d + 2));
      ctx.fillStyle = (d / 2) % 2 ? 'rgba(150,190,225,.036)' : 'rgba(0,0,0,.08)';
      ctx.fillRect(0, y1, W, y0 - y1);
    }
    var vg = ctx.createRadialGradient(cx, yGL - H * 0.02, W * 0.04, cx, H * 0.62, W * 0.82);
    vg.addColorStop(0, 'rgba(0,0,0,0)'); vg.addColorStop(1, 'rgba(2,5,10,.5)');
    ctx.fillStyle = vg; ctx.fillRect(0, 0, W, H);

    ctx.lineJoin = 'round'; ctx.lineCap = 'round';
    var w1 = Math.max(1.3, W * 0.0032);
    ...
    ctx.fillStyle = 'rgba(150,185,215,.06)';
    ctx.beginPath();
    ctx.moveTo(flLt[0], flLt[1]); ctx.lineTo(flRt[0], flRt[1]); ctx.lineTo(blRt[0], blRt[1]); ctx.lineTo(blLt[0], blLt[1]); ctx.closePath();
    ctx.moveTo(flLt[0], flLt[1]); ctx.lineTo(flL[0], flL[1]); ctx.lineTo(blL[0], blL[1]); ctx.lineTo(blLt[0], blLt[1]); ctx.closePath();
    ctx.moveTo(flRt[0], flRt[1]); ctx.lineTo(flR[0], flR[1]); ctx.lineTo(blR[0], blR[1]); ctx.lineTo(blRt[0], blRt[1]); ctx.closePath();
    ctx.moveTo(blLt[0], blLt[1]); ctx.lineTo(blRt[0], blRt[1]); ctx.lineTo(blR[0], blR[1]); ctx.lineTo(blL[0], blL[1]); ctx.closePath();
    ctx.fill();
    ctx.strokeStyle = 'rgba(150,185,215,.16)'; ctx.lineWidth = 1;
    ...
    poly([flL, flLt, flRt, flR], 'rgba(232,242,252,.94)', Math.max(2.8, W * 0.0066));
    ctx.restore();
